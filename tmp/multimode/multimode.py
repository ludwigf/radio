...
# Generated: Sat Nov 17 12:34:04 2012
##################################################
import sys
sys.path.append("/home/frank/radio/tmp/multimode/trunk")
...
import math
import multimode_helper as mh #@UnresolvedImport
import osmosdr
import threading
import time
import wx
...
class multimode(grc_wxgui.top_block_gui):
    ...
    def __init__(self, ahw="default", ppm=0.0, vol=1.0, ftune=0.0, xftune=0.0, srate=1.0e6, devinfo="rtl=0", agc=0, arate=48.0e3, upce=0, mthresh=-10.0, offs=50.e3, dfifo="multimode_fifo", mbw=2.0e3, deemph=75.0e-6, dmode="NFM1", freq=147.3e6):
        ...
        self.cur_freq = cur_freq = mh.scan_freq_out(False,150e6,300e6,freq,ifreq,scan_power+1.0e-14,thresh,12500,scan_rate,False,None)
        ...
        self.GridAdd(self._zoom_chooser, 1, 4, 1, 1)
        ...
        self.GridAdd(self._variable_static_text_0_0_static_text, 1, 5, 1, 1)
        ...
        self.osmosdr_source_c_0.set_center_freq(cur_freq+offset, 0)
    ...
    def set_freq(self, freq):
        self.freq = freq
        self.set_cur_freq(mh.scan_freq_out(False,150e6,300e6,self.freq,self.ifreq,self.scan_power+1.0e-14,self.thresh,12500,self.scan_rate,False,None))
    ...
    def set_offset(self, offset):
        self.offset = offset
        self.gr_freq_xlating_fir_filter_xxx_0_1.set_center_freq((self.offset+self.fine+self.xfine)/(self.samp_rate/1.0e6))
        self.osmosdr_source_c_0.set_center_freq(self.cur_freq+self.offset, 0)
...
if __name__ == '__main__':
    ...
    parser.add_option("", "--freq", dest="freq", type="eng_float", default=eng_notation.num_to_str(147.3e6),
        help="Set Center Frequency [default=%default]")
    (options, args) = parser.parse_args()
    tb = multimode(ahw=options.ahw, ppm=options.ppm, vol=options.vol, ftune=options.ftune, xftune=options.xftune, srate=options.srate, devinfo=options.devinfo, agc=options.agc, arate=options.arate, upce=options.upce, mthresh=options.mthresh, offs=options.offs, dfifo=options.dfifo, mbw=options.mbw, deemph=options.deemph, dmode=options.dmode, freq=options.freq)
    tb.Run(True, 300)
